...
export const addMultipleAvailabilities = async (dataArray, connection = null) => {
  ...
  try {
    ...
    dataArray.forEach((entry) => {
      const availabilityId = generateUuidV7Binary();
      const availabilityIdBinary = uuidToBinary(availabilityId);
...
export const getAvailabilitiesByUserId = async (userId) => {
...
export const updateAvailability = async (availabilityId, updatedData) => {
  const connection = await pool.getConnection();
  try {
    const availabilityIdBinary = uuidToBinary(availabilityId);
...
export const deleteAvailabilityModel = async (availabilityId, softDelete = true) => {
  const connection = await pool.getConnection();
  try {
    const availabilityIdBinary = uuidToBinary(availabilityId);
